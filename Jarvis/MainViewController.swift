...
{ //4차 산업혁명 전체 뉴스 뷰
    ...
    @objc let display = 1
    @objc var pageCount : Int = 1
    @objc var pageControl = FlexiblePageControl()
    @objc var manualBtn = UIButton()
    
    
    @objc var back = UIImageView()
    @objc var webView = UIWebView()
    @objc var monitor = UIImageView()
...
//            pageControlpageCount - 1
    ...
    override func viewDidLoad() {
        ...
        pageControl.frame = CGRect(x: 167, y: 30, width: 39, height: 37)
    ...
    @objc func Search(_ pageCount : Int) {
