...
class LoginViewController: UIViewController, SFSpeechRecognizerDelegate, UIPopoverPresentationControllerDelegate {//로그인 뷰
    ...
    override func viewDidLoad() {
        ...
        segement.setTitleTextAttributes([NSFontAttributeName : UIFont(name: "THECandybar", size: 20)!], for: .normal)
    ...
    @objc func popOver(_ send : UIButton) {
    ...
    @objc func reset(email : String)  { // 이메일 찾기 > 재설정
    ...
    @objc func loop(_ text : String, _ List : [String])-> Bool {
        var bool : Bool = false
        for key in List {
